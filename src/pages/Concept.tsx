const Concept = () => {
  return (
    <main>
      {/* Concept 1 */}
      <section id="concept1">
        <div className="concept1-inner">
          <div className="concept1-header">
            <h2>1. 学生団体って？</h2>
          </div>
          <div className="concept1-content">
            <p>
              <span className="concept_blue">学生団体</span>
              は大学などの学生コミュニティ内で
              <br />
              形成される組織で、さまざまな目的や
              <br />
              関心ごとに基づいて活動する集団です。
              <br />
              これらの団体は、学生が
              <span className="concept_blue">自己成長</span>や
              <span className="concept_blue">社会貢献</span>を促進し、
              <br />
              興味や才能を発揮する場を提供することを
              <br />
              目指しています。
            </p>
          </div>
        </div>
        <div
          className="concept1-rect-left-bottom"
          style={{
            backgroundImage: "url('/assets/img/concept/family.png')",
          }}
        />
        <div
          className="concept1-rect-right-top"
          style={{
            backgroundImage: "url('/assets/img/concept/hatena3.png')",
          }}
        />
        <div
          className="concept1-rect-right-bottom"
          style={{
            backgroundImage: "url('/assets/img/concept/triangle2.png')",
          }}
        />
      </section>

      {/* Concept 2 */}
      <section id="concept2">
        <div
          className="concept2-inner"
          style={{
            backgroundImage: "url('/assets/img/concept/concept3.png')",
          }}
        >
          <div className="concept2-header">
            <h2>2. CSFとは!?</h2>
          </div>
          <div className="concept2-content">
            <p>
              廃校文化祭実行委員会CSFとは、
              <br />
              <span className="concept_blue">
                『廃校を通して都市と地方の架け橋』
              </span>
              <br />
              を理念としている学生団体です。
              <br />
              毎年池袋の廃校
              <span className="concept_blue">みらい館大明</span>
              で<br />
              <span className="concept_blue">校境なき文化祭</span>
              を開催します。
              <br />
              CSFは廃校を通した地域の活性化
              <br />
              について考えるきっかけを提供する
              <br />
              活動をしています。
            </p>
          </div>
        </div>
        <div
          className="concept2-rect-left-bottom"
          style={{
            backgroundImage: "url('/assets/img/concept/hatena3.png')",
          }}
        />
        <div
          className="concept2-rect-right-bottom"
          style={{
            backgroundImage: "url('/assets/img/concept/grandpa2.png')",
          }}
        />
      </section>

      {/* Concept 3 */}
      <section id="concept3">
        <div className="concept3-inner">
          <div className="concept3-header">
            <h2>3. 代表あいさつ</h2>
          </div>
          <div className="concept3-content">
            <div
              className="concept3-img"
              style={{
                backgroundImage: "url('/assets/img/concept/ozeki.png')",
              }}
            />
            <div className="concept3-sentence">
              <p>
                こんにちは!!CSF代表の
                <span className="concept_blue">小瀬木</span>
                です。
                <br />
                HPをご覧いただき、ありがとうございます。
                <br />
                <br />
                廃校活用という分野が
                <br />
                もっと多くの人々が
                <span className="concept_blue">廃校の面白さ</span>
                に<br />
                触れ合えるよう私たちは日々活動を行っています。
                <br />
                <br />
                <span className="concept_blue">廃校</span>
                を用いてイベントを企画する、
                <br />
                こんな楽しさに満ちあふれた
                <br />
                団体は他にはありません。
                <br />
                <br />
                <span className="concept_blue">「東京に廃校！？」</span>
                と気になった皆さま、
                <br />
                入団希望のお問い合わせ、
                <br />
                <span className="concept_blue">文化祭のご来場</span>
                お待ちしております!!
              </p>
              <h3>CSF第14期代表　 小瀬木 遼</h3>
            </div>
          </div>
        </div>
        <div
          className="concept3-rect-left-bottom"
          style={{
            backgroundImage: "url('/assets/img/concept/square3.png')",
          }}
        />
        <div
          className="concept3-rect-right-bottom"
          style={{
            backgroundImage: "url('/assets/img/concept/gold.png')",
          }}
        />
      </section>

      {/* Concept 4 */}
      <section id="concept4">
        <div className="concept4-inner">
          <div className="concept4-header">
            <h2>4. 校境なき文化祭</h2>
          </div>
          <div className="concept4-content">
            <div
              className="concept4-container img"
              style={{
                backgroundImage: "url('/assets/img/concept/concept4_1.png')",
              }}
            />
            <div
              className="concept4-container sentence"
              style={{
                backgroundImage: "url('/assets/img/concept/concept4_2.png')",
              }}
            />
            <div
              className="concept4-container img"
              style={{
                backgroundImage: "url('/assets/img/concept/concept4_3.png')",
              }}
            />
            <div
              className="concept4-container sentence"
              style={{
                backgroundImage: "url('/assets/img/concept/concept4_4.png')",
              }}
            />
            <div
              className="concept4-container img-big"
              style={{
                backgroundImage: "url('/assets/img/concept/concept4_5.png')",
              }}
            />
            <div
              className="concept4-container sentence"
              style={{
                backgroundImage: "url('/assets/img/concept/concept4_6.png')",
              }}
            />
            <div
              className="concept4-container img"
              style={{
                backgroundImage: "url('/assets/img/concept/concept4_7.png')",
              }}
            />
            <div
              className="concept4-container sentence"
              style={{
                backgroundImage: "url('/assets/img/concept/concept4_8_2.png')",
              }}
            />
            <div
              className="concept4-container img"
              style={{
                backgroundImage: "url('/assets/img/concept/concept4_9.png')",
              }}
            />
          </div>
        </div>
        <div
          className="concept4-rect-left-top"
          style={{
            backgroundImage: "url('/assets/img/concept/ghost2.png')",
          }}
        />
        <div
          className="concept4-rect-right-bottom"
          style={{
            backgroundImage: "url('/assets/img/concept/dress.png')",
          }}
        />
      </section>
    </main>
  );
};

export default Concept;
